...
/**
 * The test bed for CDA-CH-EMED PML documents.
 */
...
class PmlTest {
    ...
    PmlTest() throws Exception {
        ...
        final File schematronFile =
            new File(this.classLoader.getResource("cdachemed/v2.0/cdachemed-PML.sch").getFile());
        final SchematronDefinition definition = definitionParser.parse(schematronFile);

        this.schematronixFile = new File(this.classLoader.getResource("cdachemed/v2.0/").getPath() + "schematronix_pml.sch");
    ...
    void testSchematronix() throws Exception {
        ...
        assertEquals("template-2.16.756.5.30.1.1.10.1.13-2018-01-22T151726",
        ...
        assertEquals("Schematron file for  - Medication List document", document.getElementsByTagName("title").item(0).getTextContent());
    }

    /**
     * Test that the Schematronix validation matches the reference validation report, 'pml1_report.html'.
     */
    @Test
    void testPml1() throws Exception {
        final File pmlFile =
            new File(Objects.requireNonNull(this.classLoader.getResource("cdachemed/pml/pml1.xml")).getFile());
        final SchematronixValidator validator = new SchematronixValidator(new StreamSource(pmlFile), this.schematronixFile);
